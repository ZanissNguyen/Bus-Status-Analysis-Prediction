...
> Tài liệu này mô tả chi tiết từng bước xử lý dữ liệu GPS xe buýt TP.HCM, đi từ file JSON gốc đến dataset Silver đã sẵn sàng phân tích. Bao gồm 3 pipeline: **Crawl → Bronze → Silver**.
...
|--------|--------|---------------|--------|
| 0 | `crawl_bus_station.py` | Trình duyệt Chromium + Internet | `data/1_bronze/bus_station.json` |
| 1 | `1_bronze.py` | Thư mục `data/bus_gps/` chứa 85 file JSON | `data/1_bronze/data_raw.parquet` |
| 2 | `2_silver.py` | Output của Pipeline 0 + Pipeline 1 | `data/2_silver/bus_gps_data.parquet`, `data/2_silver/bus_station_data.json` |

---

## 2. Pipeline 0: Crawl Trạm xe buýt

**File:** `pipelines/crawl_bus_station.py`
...
```mermaid
sequenceDiagram
    participant Script as crawl_bus_station.py
...
# Bước 1: Tạo Bronze
python pipelines/1_bronze.py

# Bước 2: Tạo Silver
python pipelines/2_silver.py
```
...
|-----|-------------|----------------|
| `PathNotFoundError: data/1_bronze/bus_station.json` | Chưa chạy Pipeline 0 | Chạy `crawl_bus_station.py` trước |
| `PathNotFoundError: data/1_bronze/data_raw.parquet` | Chưa chạy Pipeline 1 | Chạy `1_bronze.py` trước |
