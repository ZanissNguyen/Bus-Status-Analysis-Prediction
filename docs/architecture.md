...
│  ml_gold_data.parquet│          │  3_gold/inferred_route_data.json  │
...
└──────────┬───────┘         │  Bunching / Gapping Analysis          │
           │                 │  data/bunching.parquet (stop_events)  │
           └─────────────────┤  data/domino_rules.parquet            │
                             └──────────────┬────────────────────────┘
                                            │
                                            ▼
...
```mermaid
flowchart TD
    A["📡 GPS JSON Files\nsub_raw_104 → sub_raw_189"] --> B["bronze_pipeline.py\nGộp & Lưu Parquet"]
    B --> C["data_raw.parquet\n[Bronze]"]
    C --> D["silver_pipeline.py\nLàm sạch + BallTree Mapping"]
    D --> E["bus_gps_data.parquet\n[Silver]"]
    E --> F["dm_gold_pipeline.py\nFP-Growth + Route Inference"]
    E --> G["ml_gold_pipeline.py\nNén chặng + Haversine"]
    F --> H["dm_gold_data.parquet\ninferred_route_data.json\nblack_spot.parquet"]
    G --> I["ml_gold_data.parquet"]
    H --> J["bunching_pipeline.py\nBunching + Domino Mining"]
    J --> K["bunching.parquet\ndomino_rules.parquet"]
...
#### Nhánh Gold-ML (`ml_gold_pipeline.py`)
...
#### Nhánh Gold-DataMining (`dm_gold_pipeline.py`)

| File | Định dạng | Mô tả |
|------|-----------|-------|
| `3_gold/dm_gold_data.parquet` | Parquet | GPS Silver + trip_id + inferred_route + avg_speed |
| `3_gold/inferred_route_data.json` | JSON | Kết quả nhận diện tuyến của từng xe |
| `data/black_spot.parquet` | Parquet | Các điểm GPS đang kẹt xe (black spot) |
| `data/bunching.parquet` | Parquet | Sự kiện dừng đỗ tại trạm (stop events) với các cờ bunching/gapping |
| `data/domino_rules.parquet` | Parquet | Danh sách chuỗi hiệu ứng domino lây lan giữa các trạm |

---

## 4. Các Pipeline Xử lý

### 4.0 Pipeline Crawl — `pipelines/crawl_bus_station_pipeline.py`
...
### 4.1 Pipeline Bronze — `pipelines/bronze_pipeline.py`
...
### 4.2 Pipeline Silver — `pipelines/silver_pipeline.py`
...
### 4.3 Pipeline Gold ML — `pipelines/ml_gold_pipeline.py`
...
### 4.4 Pipeline Gold Data Mining — `pipelines/dm_gold_pipeline.py`
...
### 4.5 Pipeline Bunching — `pipelines/bunching_pipeline.py`
